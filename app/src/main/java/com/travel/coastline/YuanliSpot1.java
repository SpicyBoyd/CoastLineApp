...
public class YuanliSpot1 extends FragmentActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState){
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_yuanli_spot1);

        // 讀取折疊 Toolbar 物件
        CollapsingToolbarLayout collapsingToolbar =
                (CollapsingToolbarLayout) findViewById(R.id.collapsing_toolbar);
        // 設定折疊 Toolbar 的標題
        collapsingToolbar.setTitle("苑裡車站");
        ...
        mTabHost.addTab(mTabHost.newTabSpec("one")
                        .setIndicator("資訊")
                ,YuanliSpot1I.class,null);

        //同上方Tab設定，不同處為帶入參數的差異
        mTabHost.addTab(mTabHost.newTabSpec("two")
                        .setIndicator("照片")
                ,YuanliSpot1P.class, null);
    }

    public void checkout(View view) {
        //設定查詢地址
        Uri uri=Uri.parse("geo:0,0?q="+"苗栗縣苑裡鎮苑北里中山路165號苑裡車站");
        //啟動元件
