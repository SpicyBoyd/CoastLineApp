...
public class ShaluSpot1 extends FragmentActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState){
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_shalu_spot1);

        // 讀取折疊 Toolbar 物件
        CollapsingToolbarLayout collapsingToolbar =
                (CollapsingToolbarLayout) findViewById(R.id.collapsing_toolbar);
        // 設定折疊 Toolbar 的標題
        collapsingToolbar.setTitle("沙鹿車站");
        ...
        mTabHost.addTab(mTabHost.newTabSpec("one")
                        .setIndicator("資訊")
                ,ShaluSpot1I.class,null);

        //同上方Tab設定，不同處為帶入參數的差異
        mTabHost.addTab(mTabHost.newTabSpec("two")
                        .setIndicator("照片")
                ,ShaluSpot1P.class, null);
    }

    public void checkout(View view) {
        //設定查詢地址
        Uri uri=Uri.parse("geo:0,0?q="+"臺中市沙鹿區中正街94號");
        //啟動元件
